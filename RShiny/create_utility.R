  ...
  for (k in seq_along(states)) {
    ...
    for (j in seq_along(actions)) {
      ...
      if(penaltype == 'Linear'){
        ...
        utility[k,j] <- sum(transition[k,,j]*reward_penalize_linear(states,reward_sig,loss_params, 
                                                                    linear_scale, actions[j]))
      } else if(penaltype == 'Nonlinear'){
        
        utility[k,j] <- sum(transition[k,,j]*reward_penalize_high(states,reward_sig,loss_params,
  ...
  for (k in seq_along(states)) {
    ...
    for (j in seq_along(actions)) {
      ...
      if(penaltype == 'Linear'){
        ...
        utility[k,j] <- sum(transition[k,,j]*reward_penalize_linear(states,reward_exp,loss_params, 
                                                                    linear_scale, actions[j]))
      } else if(penaltype == 'Nonlinear'){
        
        utility[k,j] <- sum(transition[k,,j]*reward_penalize_high(states,reward_exp,loss_params,
                                                                  linear_scale,action_intersect,actions[j]))
      }
