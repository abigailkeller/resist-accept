##########################################
# Create stationary policies -- appendix #
##########################################
...
# transition matrices
transition_filenames <- c("20240104_TM_r1_imm6.0.1_gamma.rds",
                          "20240104_TM_r1_imm30.0.03_gamma.rds",
                          "20240104_TM_r1_imm52.0.025_gamma.rds",
                          "20240104_TM_r1_imm10.5.0.11_gamma.rds",
                          "20240104_TM_r1_imm17.4.0.04_gamma.rds",
                          "20240104_TM_r1_imm94.0.013_gamma.rds"
                          )
...
# function for getting utility matrix filenames
...
# loop through transition matrices
...
mu0.6 <- ggplot()+
  ...
  labs(x='',y='main parameterization\noptimal removal effort',
       color='Action\npenalty\nfunction',
       linetype='Ecological\nchange\nfunction')+
...
#r = 1, mu = 0.7
mu0.7 <- ggplot()+
  geom_line(data=out_df_long[out_df_long$imm=='imm17.4.0.04',],
            aes(x=state,y=policy,color=penal,linetype=loss),linewidth=1)+
  scale_color_manual(values=c('green4','mediumorchid'),
                     labels=c('Nonlinear','Linear'))+
  scale_linetype_manual(values=c('solid','dotted'),
                        labels=c('Exponential','Sigmoidal'))+
  scale_y_continuous(limits=c(0,1))+
  labs(x='state (species density)',y='alt. parameterization\noptimal removal effort',
       color='Action\npenalty\nfunction',
       linetype='Ecological\nchange\nfunction')+
  theme_minimal()+
  theme(legend.title.align=0.5,
...
#r = 1, mu = 1.1
mu1.1 <- ggplot()+
  geom_line(data=out_df_long[out_df_long$imm=='imm10.5.0.11',],
            aes(x=state,y=policy,color=penal,linetype=loss),linewidth=1)+
  scale_color_manual(values=c('green4','mediumorchid'),
                     labels=c('Nonlinear','Linear'))+
  scale_linetype_manual(values=c('solid','dotted'),
...
#r = 1, mu = 1.2
mu1.2 <- ggplot()+
  geom_line(data=out_df_long[out_df_long$imm=='imm94.0.013',],
            aes(x=state,y=policy,color=penal,linetype=loss),linewidth=1)+
  scale_color_manual(values=c('green4','mediumorchid'),
                     labels=c('Nonlinear','Linear'))+
  scale_linetype_manual(values=c('solid','dotted'),
...
final_alpha_plot <- mu0.6+mu1+mu1.3+mu0.7+mu1.1+mu1.2+legend+plot_layout(design=layout)
  
ggsave('figures/colonization_policies.tiff',final_alpha_plot,dpi=700,width=8,height=5)
