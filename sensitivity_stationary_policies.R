...
# Time horizon
Tmax <- c(25,50,100,150,200)

# import transition matrix
transition_filename <- "20240524_TM_4_0.125.rds"
...
out_df <- as.data.frame(matrix(NA,nrow=0,ncol=length(states)+3))
colnames(out_df) <- c('penal','loss','Tmax',states)
...
for(i in 1:length(Tmax)){
  ...
  transition <- readRDS(paste0('data/transition_matrices/',transition_filename))
  
  #get associated utility filenames
  utility_filenames <- get_utility_filenames(transition_filename)
  ...
  for(j in 1:length(utility_filenames)){
    ...
    out_df[nrow(out_df)+1,] <- c(out_penal,out_loss,Tmax[i],D)
  }
}

# save output
saveRDS(out_df,'data/tmax_sensitivity_policies.rds')
